                                    ...
                                    sensor_ValueCbParams_t* pValueParams)
{
  ...
  if (flag == 0)
  {
	  ST7735_DrawString(1, 9, "Prov   ", ST7735_BLACK);
	  flag = 1;
  }

  function_Task_Meaure();
...
//          TRACE_M(TF_SENSOR, "Temperature sensor raw value %d\r\n" , temp);
          ...
          uint16_t intHeartRate = (uint16_t)(beatAvg * 100);
          uint16_t stepTime_Sec = stepTime / 1000;
        
          pValueParams->data[0] = (uint8_t)(id_WS);
          pValueParams->data[1] = intHeartRate / 100;
          pValueParams->data[2] = intHeartRate % 100;
          pValueParams->data[3] = (uint8_t)stepCount / 100;
          pValueParams->data[4] = (uint8_t)stepCount % 100;
          pValueParams->data[5] = (uint8_t)stepTime_Sec / 100;
          pValueParams->data[6] = (uint8_t)stepTime_Sec % 100;

        
//          TRACE_M(TF_SENSOR, "Acceleration: x = %f, y = %f, z = %f\r\n",
//        		  getAccX(&mpu), getAccY(&mpu), getAccZ(&mpu));
      }
    }
  }
