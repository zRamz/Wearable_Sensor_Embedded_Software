...
uint8_t num_devs = 0;
const uint8_t max_devs = 20;
static uint8_t  dev_uuid[ESP_BLE_MESH_OCTET16_LEN];
static uint16_t server_address[20] = {ESP_BLE_MESH_ADDR_UNASSIGNED, ESP_BLE_MESH_ADDR_UNASSIGNED,
                                            ESP_BLE_MESH_ADDR_UNASSIGNED, ESP_BLE_MESH_ADDR_UNASSIGNED,
                                            ESP_BLE_MESH_ADDR_UNASSIGNED, ESP_BLE_MESH_ADDR_UNASSIGNED,
                                            ESP_BLE_MESH_ADDR_UNASSIGNED, ESP_BLE_MESH_ADDR_UNASSIGNED,
                                            ESP_BLE_MESH_ADDR_UNASSIGNED, ESP_BLE_MESH_ADDR_UNASSIGNED,
                                            ESP_BLE_MESH_ADDR_UNASSIGNED, ESP_BLE_MESH_ADDR_UNASSIGNED,
...
void append_char_arrays(char *dest, const char *src) {
    while (*dest != '\0') {
        dest++;
    }
...
void convert_item(uint8_t elem, char *string) {
    itoa(elem, string, 10); 
}

void convert_array_to_string(uint8_t *array, int array_length, char *result_string) {
    char buffer[20]; 

    for (int i = 0; i < array_length; i++) {
        convert_item(array[i], buffer);

        append_char_arrays(result_string, buffer); 

        if (i < array_length - 1) {
            append_char_arrays(result_string, ";");
        }
    }
}


static void post_rest_function(uint8_t arr[7])
{
    // for (int i = 0; i < 7; i++)
    // {
    //     printf("%u\n\r", data[i]);
    ...
    esp_http_client_config_t config_post = {
        .url = "http://192.168.109.153:5000/api/monitoring/add",
    ...
    char startMsg[] = "{\"test\":\"";
    char endMsg[] = "\"}";
    const int sensorDataSize = 7;
    // uint8_t arr[] = {85, 111, 123, 123, 123, 123, 123};
    char res[100] = "";

    convert_array_to_string(arr, 7, res); // Вызываем функцию
    
    // Расчет общего размера для новой строки
    int startMsgSize = 0;
    while (startMsg[startMsgSize] != '\0') {
        startMsgSize++;
    }

    int dataMessageSize = 0;
    while (res[dataMessageSize] != '\0') {
        dataMessageSize++;
    }

    int endMsgSize = 0;
    while (endMsg[endMsgSize] != '\0') {
        endMsgSize++;
    }

    int totalJsonSize = startMsgSize + dataMessageSize + endMsgSize + 1; // +1 для нулевого символа

    // Создаем массив достаточной длины
    char combined[totalJsonSize];

    // Копирование первой строки
    int i = 0;
    for (int j = 0; j < startMsgSize; j++) {
        combined[i++] = startMsg[j];
    }

    // Копирование второй строки
    for (int j = 0; j < dataMessageSize; j++) {
        combined[i++] = res[j];
    }

    // Копирование третьей строки
    for (int j = 0; j < endMsgSize; j++) {
        combined[i++] = endMsg[j];
    }

    // Добавление нулевого символа в конце
    combined[i] = '\0';
    ...
    esp_http_client_set_post_field(client, combined, strlen(combined));
...
//   {
//     HTTPClient http;
//     String url = String(serverAddress) + ":" + String(serverPort) + String(endpoint);
                               ...
                               uint16_t primary_addr, uint8_t element_num, uint16_t net_idx)
{
    ...
    {
        if (server_address[i] == ESP_BLE_MESH_ADDR_UNASSIGNED) 
        {
            server_address[i] = primary_addr;
            num_devs++;
            break;
        }
    }
...
void example_ble_mesh_send_sensor_message(uint32_t opcode, int ind)
{
    ...
    node = esp_ble_mesh_provisioner_get_node_with_addr(server_address[ind]);
    if (node == NULL) {
        ESP_LOGE(TAG, "Node 0x%04x not exists", server_address[ind]);
        return;
    }
...
static void example_ble_mesh_sensor_timeout(uint32_t opcode, int ind)
{
    switch (opcode) {
    ...
    // example_ble_mesh_send_sensor_message(opcode, ind);
}

static void example_ble_mesh_sensor_client_cb(esp_ble_mesh_sensor_client_cb_event_t event,
                                              esp_ble_mesh_sensor_client_cb_param_t *param, int ind)
{
    esp_ble_mesh_node_t *node = NULL;
            ...
            if (param->status_cb.sensor_status.marshalled_sensor_data->len) {
                ...
                for (; length < param->status_cb.sensor_status.marshalled_sensor_data->len; ) {
                    ...
                    if (data_len != ESP_BLE_MESH_SENSOR_DATA_ZERO_LEN) {
                        ...
                        uint8_t res[7];
                        memcpy(res, data + mpid_len, sizeof(uint8_t) * 7);
                        post_rest_function(res);
                        for (int i = 0; i < 7; i++)
                        {
                            printf("%u\n\r", res[i]);
                        }
    ...
    case ESP_BLE_MESH_SENSOR_CLIENT_TIMEOUT_EVT:
        example_ble_mesh_sensor_timeout(param->params->opcode, ind);
    default:
        break;
    }
}
...
void example_task(void *pvParameter) {
    int num_devs = *((int *)pvParameter);
        for (int i = 0; i < num_devs; i++) {
            example_ble_mesh_send_sensor_message(ESP_BLE_MESH_MODEL_OP_SENSOR_GET, i);
        }
        vTaskDelay(pdMS_TO_TICKS(5 * 1000)); // Задержка в 5 секунд
        vTaskDelete(NULL);
}
...
void timer_callback(TimerHandle_t xTimer) {
    // Этот код будет выполняться каждые 5 секунд
...
void app_main(void)
{
        ...
        TimerHandle_t xTimer = xTimerCreate("Timer",              // Имя таймера
                                         5000 / portTICK_PERIOD_MS,  // Период в тиках
                                         pdTRUE,              // Автоперезапуск таймера
                                         NULL,                // Идентификатор колбэка
